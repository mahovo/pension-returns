...
risk_percentiles_df <-  data.frame(
      ...
      Velliv_m = risk_percentiles(fit_vmr$quantile_data, percent_vals, "max"),
      Velliv_m_l = risk_percentiles(fit_vmrl$quantile_data, percent_vals, "max"),
      Velliv_h = risk_percentiles(fit_vhr$quantile_data, percent_vals, "max"),
...
- Importance Sample seems to converge to a lower level than Monte Carlo does. Is that because IS catches more observations in the lower tail? Or a mistake...?  
- mc_simulation() and importance_sampling() and is_proposal() subtracts 1 from num_periods. We shouldn't do that.  
- The QQ lines look a bit off... What's happening there?  
