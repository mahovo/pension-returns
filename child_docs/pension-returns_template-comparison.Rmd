...
The fits in this section are based on **n = `r length(fits[[1]]$data)`** observations.
...
```{r}
fit_se_summary <- sapply(fits, function(f) sstd_se(f$data)$se)
colnames(fit_se_summary) <- fund_names
knitr::kable(fit_se_summary, digits = 4)
```

```{r}
## Quantities for the paragraph below; kept correct for any sample size / fund set.
.n_obs    <- length(fits[[1]]$data)
.est      <- fit_summary[c("m", "s", "nu", "xi"), , drop = FALSE]
.nu_relse <- 100 * fit_se_summary["nu", ] / as.numeric(.est["nu", ])
.nu_relse <- .nu_relse[is.finite(.nu_relse)]
.n_sing   <- sum(!is.finite(fit_se_summary["nu", ]))
.fin      <- which(is.finite(fit_se_summary["nu", ]))[1]   # a fund whose nu is identified
.rf       <- fund_names[.fin]
.rf_nu    <- as.numeric(.est["nu", .fin]); .rf_nu_se <- fit_se_summary["nu", .fin]
.rf_nu_lo <- .rf_nu - 1.96 * .rf_nu_se;    .rf_nu_hi <- .rf_nu + 1.96 * .rf_nu_se
.rf_xi    <- as.numeric(.est["xi", .fin]); .rf_xi_se <- fit_se_summary["xi", .fin]
.rf_xi_lo <- .rf_xi - 1.96 * .rf_xi_se;    .rf_xi_hi <- .rf_xi + 1.96 * .rf_xi_se
```
...
tail index `nu` and skew `xi` are another matter. `r if (.n_sing > 0) paste0("For ", .n_sing, " of the ", length(fits), " funds the information matrix is singular, so nu and xi are not even locally identified. ")`Where `nu` is identified its relative standard error
ranges from `r round(min(.nu_relse))`% to `r round(max(.nu_relse))`%; for `` `r .rf` `` the
95% interval is approximately [`r round(.rf_nu_lo, 1)`, `r round(.rf_nu_hi, 1)`], which
...
the return variance is even finite. The skew for `` `r .rf` `` is `xi` = `r round(.rf_xi, 2)`
with 95% interval [`r round(.rf_xi_lo, 2)`, `r round(.rf_xi_hi, 2)`]`r if (is.finite(.rf_xi_hi) && .rf_xi_hi < 1) ", which lies entirely below 1, so the left skew is statistically established"`.
...
make_extreme_probs <- function(data_df) {
  ...
  for(i in 1:ncol(data_df)) {
    x <- data_df[, i]
    extreme_probs[, i] <- c(
      pnorm(min(x), fits_norm[[i]]$dist_params[1], fits_norm[[i]]$dist_params[2]) * 100,
