...
risk_percentiles_df <- as.data.frame(lapply(
  fits,
  function(fit) {
    risk_percentiles(fit$quantile_data, percent_vals, "max")
  }
))
...
gain_percentiles_df <- as.data.frame(lapply(
  fits,
  function(fit) {
    risk_percentiles(fit$quantile_data, percent_vals, "min")
  }
))
...
mc_loss_percentiles_df <- as.data.frame(lapply(
  mc_output,
  function(x) {
    risk_percentiles(log(unlist(x$mc_df[20, ])/100), percent_vals, "max")
  }
))
...
mc_gain_percentiles_df <- as.data.frame(lapply(
  mc_output,
  function(x) {
    risk_percentiles(log(unlist(x$mc_df[20, ])/100), percent_vals, "min")
  }
))
