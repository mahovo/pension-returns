---
title: "An informal study of the skewed generalized t-distribution"
...
Try convolution with `bayesmeta::convolve`.

This works, but with `dsstd()` it only with one iteration. In other words, self-convolution is not possible.
...
Does not fail but does also not complete en reasonable time:

```{r eval=FALSE}
sim_params <- c(0.08, 0.12)
num_periods <- 4
```
...
Conclusion for now: It might be interesting to compare convolution with estimating the density of Sn in terms of accuracy vs performance. But for now, self-convolution does not seem feasible with this approach (when we don't have a closed form). Even just a few itarations take a long time.
