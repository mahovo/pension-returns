...
sharpe   <- function(x) mean(x) / sd(x) * sqrt(12)   # excess return ~ return (rates ~ 0 here)
```
...
ggplot(mv, aes(vol, ret, colour = provider)) +
  geom_abline(slope = mean(sapply(R[1:3], sharpe)), intercept = 0,
...
```{r divers-table}
divr <- function(a, b) (0.5 * sd(a) + 0.5 * sd(b)) / sd(0.5 * a + 0.5 * b)  # >1 = risk reduced
mixes <- list(
...
.H <- .simp(R$P_D); .L <- 2 * .simp(R$P_B) - .simp(R$P_D)
.divr <- function(a, b) (0.5 * sd(a) + 0.5 * sd(b)) / sd(0.5 * a + 0.5 * b)
knitr::kable(
  data.frame(
    blend = c("Within PFA: bond fund + equity fund (a mid-risk plan)",
...
```{r prop-table}
W2 <- c(1e5, 3e5, 1e6, 3e6); F2v <- c(500, 1000, 2000)
