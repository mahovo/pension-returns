...
```{r sharpe-ray-curve}
.sharpe <- function(x) mean(x) / sd(x)   # annual data: already annualised; rates ~ 0
.yrs    <- 2011:2023
.from   <- c(2011, 2014, 2017)
.plans  <- c(vmr = "Velliv medium", vhr = "Velliv high",
             pmr = "PFA medium",    phr = "PFA high")
.tab <- t(sapply(.from, function(f) {
  .s <- data_df[.yrs >= f, ]
  round(sapply(names(.plans), function(p) .sharpe(.s[[p]])), 2)
}))
rownames(.tab) <- paste0("from ", .from, "  (n=", sapply(.from, function(f) sum(.yrs >= f)), ")")
colnames(.tab) <- .plans
knitr::kable(.tab, caption = "Annualised Sharpe ratio by plan and start year (annual returns).")
```
